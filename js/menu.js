...
export const logoffOpen = function () {
  ...
  const buttons = {
    cancel: {
      ...
      className:
        "d-xl-block d-lg-block d-md-block d-none btn-outline-secondary",
    },
    logOff: {
  ...
  if (!menuconfig.disconnect) {
    console.error(
      "menuconfig.disconnect is not defined, or disabled by config file"
    );
    return;
  }
...
export const init = function () {
  launcher.getkeyinfo("menuconfig").done((ret) => {
    if (ret.id) {
      menuconfig = ret.id;
        ...
        documentmenuentries.forEach((elt) => {
          ...
          if (menuentryid) {
            if (!menuconfig[menuentryid]) {
              ...
              const indexDivider =
                Array.from(dropDown.children).indexOf(li) + 1;
