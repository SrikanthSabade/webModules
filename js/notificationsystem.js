...
export const displayNotification = function (
  ...
  if (mydelay < 0) {
    ...
    if (already_show_delay_in_milliseconds === null)
      ...
      already_show_delay_in_milliseconds = parseInt(
        already_show_delay_in_milliseconds
      );
      if (already_show_delay_in_milliseconds <= 0) return;
      mydelay = already_show_delay_in_milliseconds;
      // show four times
      already_show_delay_in_milliseconds =
        already_show_delay_in_milliseconds -
        default_max_duration_in_milliseconds / 4;
...
export const showNotification = function (notif) {
  ...
  divSecondCol.className =
    "col-xl-2 col-lg-2 d-xl-flex d-lg-flex d-none justify-content-center align-items-center close-notification";
  ...
  if (panel) {
    ...
    Array.from(document.querySelectorAll(".closeNotif"))
      ...
      .forEach((close) =>
        close.click(() =>
          $(`#${close.data("notif")}`).hide("slide", { direction: "right" })
        )
      );
...
broadcastEvent.addEventListener("hello", ({ detail: { user } }) => {
  ...
  const picture = user.photo
    ? `data:;base64,${user.photo}`
    : window.od.net.urlrewrite("../img/top/place-available.svg");
  displayNotification(
    user.name,
    languages.getTranslate("connected"),
    "",
    picture
  );
});
...
broadcastEvent.addEventListener("bye", ({ detail: { user } }) => {
  ...
  displayNotification(
    user.name,
    languages.getTranslate("disconnected"),
    "",
    picture
  );
});
